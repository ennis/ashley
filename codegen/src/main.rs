...
//--------------------------------------------------------------------------------------------------
// Utilities
fn to_ident(ident: impl AsRef<str>) -> Ident {
...
fn generate_ext_impl_block(grammar: &ExtGrammar, ext_name: &str, ext_prefix: &str) -> proc_macro2::TokenStream {
    generate_instruction_methods(&grammar.instructions, Some((ext_name, ext_prefix)))
}

//--------------------------------------------------------------------------------------------------

fn generate_decoration_enum() {}
...
fn main() {
    // --- HIR instruction builders ---
    {
        ...
        for (ext, ext_prefix, file) in EXT_INST_GRAMMARS {
            ...
            let impl_block = generate_ext_impl_block(&ext_grammar, ext, ext_prefix);
            write!(output, "\n// --- {} --- \n{impl_block}\n", ext).unwrap();
        }
