...
const EXT_INST_GRAMMARS: &[(Extension<'static>, &str)] = &[(
    Extension {
        name: "GLSL.std.450",
        mnemonic: "glsl",
    },
    "codegen/extinst.glsl.std.450.grammar.json",
)];
const OUTPUT: &str = "ashley/src/hir/autogen/builder.rs";
...
fn generate_instruction(inst: &Instruction, ext: Option<Extension>) -> proc_macro2::TokenStream {
    ...
    let method_name = to_ident(if let Some(ext) = ext {
        format!("emit_{}_{opname}", ext.mnemonic)
    } else {
        format!("emit_{opname}")
    });
...
fn generate_instruction_methods(insts: &[Instruction], ext: Option<Extension>) -> proc_macro2::TokenStream {
    let mut methods = vec![];
    for inst in insts.iter() {
        // type and constant builders are hand-crafted
        if matches!(
            inst.class,
            Some(InstructionClass::TypeDeclaration | InstructionClass::ConstantCreation | InstructionClass::Function)
        ) {
            continue;
        }
...
fn generate_impl_block(grammar: &Grammar) -> proc_macro2::TokenStream {
    generate_instruction_methods(&grammar.instructions, None)
}

fn generate_ext_impl_block(grammar: &ExtGrammar, ext: Extension) -> proc_macro2::TokenStream {
    generate_instruction_methods(&grammar.instructions, Some(ext))
}

fn load_grammar_file(path: &str) -> Grammar {
    let file = File::open(path).expect(&format!("failed to open grammar file: `{}`", path));
    let reader = BufReader::new(file);
    json::from_reader(reader).expect("failed to parse grammar file")
}

fn load_ext_grammar_file(path: &str) -> ExtGrammar {
    let file = File::open(path).expect(&format!("failed to open grammar file: `{}`", path));
    let reader = BufReader::new(file);
    json::from_reader(reader).expect("failed to parse grammar file")
}
...
fn main() {
    ...
    {
        let core_grammar = load_grammar_file(CORE_GRAMMAR);
        let impl_block = generate_impl_block(&core_grammar);
        write!(output, "// --- Core instructions --- \n{impl_block}\n").unwrap();
    }

    for (ext, file) in EXT_INST_GRAMMARS {
        let ext_grammar = load_ext_grammar_file(*file);
        let impl_block = generate_ext_impl_block(&ext_grammar, *ext);
        write!(output, "\n// --- {} --- \n{impl_block}\n", ext.name).unwrap();
    }
}
