...
fn main() {
    let position : vec2 = i_position * vec2(1.0, -1.0) * u_resolution * 0.5;
    let vignette : float = clamp(0.7 * length(i_position), 0.0, 1.0);
    out_color = mix(
    ...
    let grid_scale : float = 5.0;
    if u_zoom < 2.5 {
        grid_scale = 1.0;
    }
    let pos : vec2 = position + u_scroll_offset * u_zoom;
